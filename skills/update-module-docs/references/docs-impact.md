...
Shared reference for `update-module-docs` (gates: is the delta really
mechanical-only?) and `upgrade-module-docs` (scope: which category files get
...
- A file you cannot confidently map → the verdict is `affected` (over-marking
  is safe: it just triggers a re-discover; under-marking ships stale docs).
...
- **`not_applicable`** — no docs baseline in this release line (new line or
  never-discovered module).

## Contained vs sweeping (`affected` only)

The boundary between `upgrade-module-docs` and a full re-discover is the
...
- **Sweeping** — the architecture itself changed (or it's a new line's first
  ...
  Full `discover-drupal-module` is safer.

The affected-file count is a **cost signal**, not a refusal trigger: when
nearly every category is affected, upgrade's token savings over a full
discover shrink toward zero — say so in the report so the user can weigh it —
