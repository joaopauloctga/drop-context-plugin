...
description: >-
  Produce a full, AI-consumable analysis of a Drupal contrib or custom module,
  written as category Markdown files plus a metadata.json manifest under
  ~/.drop-context/docs/modules/<module>/<version>/. Accepts an optional
  version/tag parameter — when omitted, the version is resolved from the
  module's own .info.yml / composer.lock. Reads the module source directly out
  of the Drupal repo you run it in (web/modules/contrib, web/modules/custom) —
  no network access, nothing downloaded. Use
...
generated docs flow through your context. You resolve the module inside the
repo, verify the paths, spawn the explorers in parallel, then write the two
files only you can write: `summary.md` and `metadata.json`.

**Read-only contract.** This skill must be run from inside (or pointed at) an
installed Drupal repo. `MODULE_ROOT` will point at the user's real,
version-controlled module source — contrib or custom. It is **read-only**:
only `Read`/`Glob`/`grep`/`find` ever touch it. Every write, by you or by any
spawned explorer, goes **only** under `OUTPUT_DIR`, which is always inside
`~/.drop-context/docs/` (a single user-level location, independent of which
repo you're standing in — override the base with `DROP_CONTEXT_HOME`). Never
edit, move, or delete anything under `MODULE_ROOT`.

Inputs: a module **machine name** (required, e.g. `webform`), an optional
**version** (only needed when the resolver cannot find one on disk — see step
1), and an optional **submodule scope** — see "Submodule scope" at the end of
step 1. By default the run documents the root module **and** all its
submodules.

Follow these steps in order.

## 1. Resolve & GATE — one script run

**This is a hard gate. Do not spawn any explorer until it passes.** The
bundled script does the whole preparation in one run, with **zero network
access**: it locates the module inside the Drupal repo you're running in
(contrib, custom, or core), resolves its version from on-disk facts only,
creates the output directory, and enumerates submodules. It lives in
`scripts/` next to this SKILL.md — resolve `SKILL_DIR` to the **absolute path
of the directory containing this SKILL.md** (you know it from where this
skill was loaded); never assume a fixed project-relative location. The script
is standard-library-only Python: any `python3` works, nothing to install.

```bash
SKILL_DIR="<absolute path of the directory containing this SKILL.md>"
python3 "$SKILL_DIR/scripts/resolve.py" <module> --kind contrib,custom
```

**Always pass `--kind contrib,custom`** — this skill never documents a core
module (that's `/drop-context:document-core-module`'s job), and the flag makes the
resolver refuse a core module deterministically instead of documenting it
under the wrong track.
...
PROJECT=<drupal.org project name, from info.yml `project:`, or = MODULE>
KIND=contrib|custom
VERSION=<resolved from info.yml / composer.lock / core version>
MODULE_ROOT=<absolute path inside the repo — READ-ONLY, never write here>
DRUPAL_ROOT=<absolute docroot>
OUTPUT_DIR=<absolute, always under ~/.drop-context/docs/modules/<module>/<version>>
...
(`IS_SUBMODULE=no` is guaranteed here — see "Submodule-of-parent" below for
the other case, which the resolver refuses by default rather than reaching
this block.)

If the module isn't installed anywhere the script searched, or its version
can't be resolved from disk, the script instead prints an actionable
`GATE NEEDS INPUT` block. **Never guess a missing version or a missing module
path, and never silently redirect or proceed on any of these:**

- `REASON=module-not-found` → ask the user to install the module or pass
  `--path <dir>` (AskUserQuestion in Claude Code).
- `REASON=version-unresolved` → offer the script's `SUGGEST=` values
  (typically `dev` and/or a short git SHA) and let the user pick or supply
  their own label, then re-invoke with `--version <answer>`.
- `REASON=ambiguous` → show the `CANDIDATES=` paths and ask which one, then
  re-invoke with `--path <dir>`.
- `REASON=kind-mismatch` → the named module is a **core** module. Tell the
  user and point them at `/drop-context:document-core-module <module>` instead — do
  not retry with a different `--kind`, and do not document it here.
- `REASON=submodule-of-parent` → the named module is a **submodule** of
  `PARENT=<parent machine name>` (found nested under `PARENT_ROOT=`). Tell
  the user it belongs to `<parent>` and that
  `/drop-context:document-module <parent>` documents it as
  `submodules/<module>.md` alongside the rest of that project — this is
  almost always what they want. **Ask** (AskUserQuestion) whether to (a) run
  the parent discover instead (recommended — offer to do it), or (b) proceed
  with a standalone doc set for just this submodule anyway. Only on an
  explicit (b) answer, re-invoke with `--allow-submodule-standalone` and
  continue this skill normally from the `GATE OK` it then returns
  (`IS_SUBMODULE=yes`/`PARENT=`/`PARENT_ROOT=` will be present in that block —
  carry them into `metadata.json` in step 7 so the standalone doc set records
  its relationship rather than looking like an independent project). Never
  choose (b) on the user's behalf.

If the script exits non-zero with `GATE FAILED` (no Drupal repo found, or an
internal error), **stop and report its error message** — do not spawn
explorers, and do not improvise a different path or search elsewhere on the
filesystem.
...
```bash
python3 "$SKILL_DIR/scripts/verify.py" "$OUTPUT_DIR" --partial --module <module> --module-root "$MODULE_ROOT"
```
...
(Unix epoch seconds). `type` is `"contrib"` or `"custom"`, copied from the
gate's `KIND` (a core module — `KIND=core` — is handled by the
`/drop-context:document-core-module` variant of this skill instead). This skill's
`metadata.json` carries no `project` key — the discover pipeline no longer
fetches Drupal.org project metadata; it is zero-network by design.

**Standalone submodule (the rare, explicitly-confirmed `--allow-submodule-standalone` case
from step 1 only)**: add a top-level `"submodule_of": {"module": "{PARENT}", "path": "{PARENT_ROOT}"}`
key, copied verbatim from the gate's `PARENT=`/`PARENT_ROOT=` lines, so the
doc set records that this is a submodule documented on its own rather than
looking like an independent project. Omit the key entirely in the normal case
(`IS_SUBMODULE=no`).
...
unlisted files; valid categories; submodule count matching this run's scope),
runs the doc-only consistency checks, and — via `--module-root` — grounds the
docs in the source:
