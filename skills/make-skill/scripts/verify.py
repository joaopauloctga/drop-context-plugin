#!/usr/bin/env python3
"""Verify a generated dc-* skill against the doc set it was built from
(step 8 of make-skill / make-core-skill).
...
Grounding checks (the anti-fabrication net): the doc set is the
...
the skill, never the doc set, so a sentence that points it at the
doc set ("see the doc set's Entities category", "full catalog in
`hooks.md` of the doc set", a bare `plugins.md` with no such sibling
reference) is a dangling pointer — a PROBLEM. Any other mention of the
doc set (hedges like "the doc set does not enumerate …") is a
...
in the doc set are WARNINGs — invented enum values and guessed ids are
...
# --- consumer-contract: dangling pointers at the doc set -------------------
DOC_SET_CATEGORY_FILES = (
    "summary",
    "entities",
    "plugins",
    "services",
...
DOC_SET_RE = re.compile(
    r"\bdoc(?:umented)?[ -]set\b|\bdocument-(?:core-)?module\b",
    re.IGNORECASE,
)
# Sentences that *direct* the reader at the doc set (a pointer the
# consumer cannot follow) as opposed to merely mentioning them (a hedge).
POINTER_RES = (
    # "see the doc set", "consult the doc set for …"
    re.compile(
    ...
    # "the doc set's Entities category", "doc set (Services section)"
    re.compile(
        r"\bdoc(?:umented)?[ -]set\b['’]?s?[^.\n]{0,20}?\b"
        + _CAT_WORDS
...
DOC_SET_FILE_RE = re.compile(
    r"`(?:submodules/)?(" + "|".join(DOC_SET_CATEGORY_FILES) + r")\.md`"
)
DOC_SET_FILE_TAIL_RE = re.compile(
    r"^[^.\n]{0,30}?\b(?:of|in|from) the doc set", re.IGNORECASE
)
...
def check_dangling_pointers(
    ...
    """Pointers at the doc set → PROBLEM; other mentions → WARNING."""
    problems: list[str] = []
    warnings: list[str] = []
    for p in gen_files:
        ...
        for ln, line in prose:
            ...
            for rx in POINTER_RES:
                if rx.search(line):
                    problems.append(
                        f"{rel}:{ln}: points the consumer at the doc set "
                    ...
                    break
            if not flagged:
                for m in DOC_SET_FILE_RE.finditer(line):
                    name = m.group(1) + ".md"
                    tail = line[m.end() :]
                    if name not in sibling_refs or DOC_SET_FILE_TAIL_RE.match(tail):
                        problems.append(
                            f"{rel}:{ln}: refers to doc set file `{name}` which the "
                            "consumer cannot open (no such sibling reference): "
                            f"{line.strip()[:110]}"
                        )
                        flagged = True
                        break
            if not flagged and DOC_SET_RE.search(line):
                warnings.append(
                    f"{rel}:{ln}: mentions the doc set — the consumer cannot "
                    "see it; rephrase as a plain hedge or remove: "
                    f"{line.strip()[:110]}"
                )
            ...
            for val, ln, key in y.values:
                ...
                if not grounded(corpus, val):
                    ...
                    warnings.append(
                        ...
                        "the doc set (invented enum value / guessed id?)"
                    )
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        required=True,
        help="The doc set dir the skill was generated from (DOCS_DIR).",
    )
    ap.add_argument(
        "--name",
        default=None,
    ...
    for p in gen_files:
        ...
        for m in FQCN_RE.finditer(gtext):
            ...
            if not grounded(corpus, fqcn):
                problems.append(
                    f"{rel}: {fqcn} does not appear in the doc set — "
        ...
        for m in HOOK_RE.finditer(gtext):
            ...
            if not grounded(corpus, hook):
                problems.append(
                    f"{rel}: {hook}() does not appear in the doc set — "
                    "not derivable from them, likely invented"
                )
        for m in dotted_re.finditer(gtext):
            ...
            if not grounded(corpus, token):
                warnings.append(
                    f"{rel}: `{token}` not found in the doc set — "
