...
Bundled with the `retag-docs` skill (and used by `add-release`
via a sibling path). This is the pipeline's **only** remaining network access:
both skills diff two tags of an already-documented module, which needs both
tags' source on disk — something an installed repo (which only ever holds one
checked-out version) cannot provide. `document-module` /
`document-core-module` no longer use this script — they read the repo
you run them in instead (see `../document-module/scripts/resolve.py`).
Self-contained: standard library only — any Python 3.9+ runs it, nothing to
install.
...
so the skill can diff it without touching the user's project or home
directory. The OS reclaims the temp dir on its own schedule; re-running the
same <module>/<ref> is a cache hit (no re-download).

Besides downloading, the script validates the extracted source and enumerates
...
No Drupal.org project metadata is fetched — that lookup was removed when the
document skills went zero-network; this script only ever needed it as a
courtesy for the (now gone) `metadata.json` `project` key.
"""
...
def _gate_report(module: str, ref: str, module_root: Path, cache_dir: Path) -> None:
    """GATE: print the machine-parseable block. No network beyond the download
    itself — this script no longer fetches Drupal.org project metadata."""
    print("GATE OK")
