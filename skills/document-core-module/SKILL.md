...
description: >-
  ...
  manifest under ~/.drop-context/docs/core/<version>/<module>/. The core
  version is always resolved from the repo's own Drupal::VERSION — never a
  parameter. Reads the module source directly out of the Drupal repo you run
  it in (web/core/modules/<module>) — no network access, nothing downloaded.
  Use when
...
# Document a Drupal core module

Same shape as `/drop-context:document-module`, but the target is a **Drupal core**
module — one that ships inside `core/modules/<module>/` in the Drupal repo you
run this skill in, not a contrib project. Core modules have no standalone
project on drupal.org and no separate version of their own — the module's
version **is** the repo's Drupal core version — so this skill **resolves the
module directly from the installed checkout**, reusing the same
zero-network `resolve.py` the contrib skill bundles (`../document-module`
must be installed alongside; see step 1).
...
`drupal-module-explorer` agent** the contrib document skill uses — it just
receives a path inside `core/modules/`.

**Read-only contract.** `MODULE_ROOT` points at `core/modules/<module>/`
inside the user's real Drupal checkout. It is **read-only** — only
`Read`/`Glob`/`grep`/`find` ever touch it. Every write goes only under
`OUTPUT_DIR`, always inside `~/.drop-context/docs/` (a single user-level
location, independent of which repo you're standing in — override the base
with `DROP_CONTEXT_HOME`). Never edit, move, or delete anything under
`MODULE_ROOT` or elsewhere in `core/`.

Inputs: a core module **machine name** (required, e.g. `views`). No version
parameter — the core version is always `Drupal::VERSION` from the repo you're
running in.
...
bundled resolver — shared with `/drop-context:document-module`, at
`../document-module/scripts/resolve.py` relative to this SKILL.md —
does the whole preparation in one run, with **zero network access**: it
locates the module under `core/modules/`, reads the core version from
`Drupal::VERSION` (never the module's own `.info.yml`, which typically carries
the literal placeholder `version: VERSION`), creates the output directory, and
enumerates submodules. Resolve `SKILL_DIR` to the **absolute path of the
directory containing this SKILL.md** (you know it from where this skill was
loaded); never assume a fixed project-relative location.

```bash
SKILL_DIR="<absolute path of the directory containing this SKILL.md>"
python3 "$SKILL_DIR/../document-module/scripts/resolve.py" <module> --kind core
```

**Always pass `--kind core`** — this skill never documents a contrib or
custom module (that's `/drop-context:document-module`'s job), and the flag makes the
resolver refuse a non-core module deterministically instead of documenting it
...
GATE OK
MODULE=<machine name>
PROJECT=<= MODULE for a core module>
KIND=core
VERSION=<Drupal::VERSION of the repo you're running in>
MODULE_ROOT=<absolute path to core/modules/<module> — READ-ONLY, never write here>
DRUPAL_ROOT=<absolute docroot>
OUTPUT_DIR=<absolute, always under ~/.drop-context/docs/core/<version>/<module>>
...
(`IS_SUBMODULE=no` is guaranteed here — see below for the rare case where a
core module nests under another one.)

The script instead prints an actionable `GATE NEEDS INPUT` block on any of
these — **never guess, never silently redirect or proceed:**

- `REASON=module-not-found` → the module doesn't exist under `core/modules/`
  in this repo (usually a wrong machine name, or the target really is a
  contrib module). Tell the user and suggest `/drop-context:document-module` instead
  rather than guessing.
- `REASON=kind-mismatch` → the named module resolved to a **non-core** kind
  (`ACTUAL=contrib` or `ACTUAL=custom`). Tell the user and point them at
  `/drop-context:document-module <module>` instead — do not retry with a different
  `--kind`, and do not document it here.
- `REASON=submodule-of-parent` → the named module is nested under
  `PARENT=<parent machine name>` (a real but rare shape for a core module).
  Tell the user it belongs to `<parent>` and that
  `/drop-context:document-core-module <parent>` documents it as
  `submodules/<module>.md` — **ask** (AskUserQuestion) whether to run the
  parent document skill instead (recommended) or proceed standalone; only on an
  explicit standalone answer, re-invoke with `--allow-submodule-standalone`
  and carry the resulting `PARENT=`/`PARENT_ROOT=` into `metadata.json`'s
...
**Standalone submodule (the rare, explicitly-confirmed `--allow-submodule-standalone`
case from step 1 only)**: add a top-level `"submodule_of": {"module": "{PARENT}", "path": "{PARENT_ROOT}"}`
key, copied verbatim from the gate's `PARENT=`/`PARENT_ROOT=` lines. Omit the
key entirely in the normal case (`IS_SUBMODULE=no`).
