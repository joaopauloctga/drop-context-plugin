#!/usr/bin/env python3
"""Resolve a Drupal core library and prepare a document run.

Standard-library-only gate for ``document-core-library``::
...
class GateError(RuntimeError):
    """A preparation error that should stop the document workflow."""
...
def candidate_lib_roots(base: Path, search_ancestors: bool) -> list[Path]:
    ...
    for anchor in anchors:
        ...
        add(anchor / "docroot" / "core" / "lib" / "Drupal")
    return roots
...
def docs_root() -> Path:
    """${DROP_CONTEXT_HOME:-~/.drop-context}/docs — the root of every document/make output this pipeline writes, independent of which repo the user
    is standing in.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "${DROP_CONTEXT_HOME:-~/.drop-context}/docs/core-libraries — "
        "override mainly for isolated testing; prefer setting DROP_CONTEXT_HOME instead).",
    )
    parser.add_argument(
        "--work-base",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        core_root = lib_root.parent.parent
        output_base = (
            args.output_base.expanduser() if args.output_base else (docs_root() / "core-libraries")
        )
        output_dir = output_base / version / relative_path
