...
description: >-
  ...
  ~/.drop-context/docs/core-libraries/{core-version}/{qualified-library}/. Reads
  an installed Drupal checkout — no network access. Uses a
  ...
  under core/modules (use /drop-context:document-core-module instead).
---

# Document a Drupal core library
...
**Read-only contract.** `LIBRARY_ROOT`/`CORE_ROOT`/`DRUPAL_LIB_ROOT` point at
the user's real, version-controlled Drupal core checkout. They are
**read-only** — only `Read`/`Glob`/`grep`/`find` ever touch them. Every write,
by you or any spawned worker, goes only under `OUTPUT_DIR` (always inside
`~/.drop-context/docs/` — a single user-level location, independent of which
repo you're standing in; override the base with `DROP_CONTEXT_HOME`) or
`WORK_DIR` (a disposable OS temp directory, unrelated to that docs root).
Never edit, move, or delete anything under `core/`.
...
- an optional output language; default to English (`en`) for consistency with
  the existing documentation corpus unless the user requests another language.
...
OUTPUT_DIR=<absolute, always under ~/.drop-context/docs/core-libraries/<version>/<library path>>
...
Workers use the agent contract shipped with this plugin as
`drupal-core-library-explorer`:
