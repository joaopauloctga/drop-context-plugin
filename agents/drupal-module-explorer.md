...
- **configuration** — config object name, key settings + types (from `config/schema/`), defaults (from `config/install/`), where to configure in the UI. When naming **plugin IDs** (in examples, option lists, UI walkthroughs), take them from the plugin classes' attribute/annotation `id:` declarations — **never from config schema key names**, which can be stale or orphaned (a real module shipped a `…filter.bef_datepickers` schema key for a plugin whose actual id is `bef_datepicker`). Schema keys may be *cited as schema keys*, but a value presented as a plugin id must come from a declaration.
